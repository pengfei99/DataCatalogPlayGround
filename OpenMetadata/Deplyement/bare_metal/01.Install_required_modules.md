...
OM only support postgres 15, so don't install a newer version
...
I recommend you creat a file `db_init.sql`, and put the above content in it. Don't forget to change the password
...
The main config file `openmetadata.yaml` is located at `/path/to/openmetadata-1.x.x/conf`. In this tutorial, we download
the versio 1.9.0. So the below config example works for v1.9.0.

The minimum config is to connect om to the target `DB` and `EL`.
